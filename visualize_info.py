...
def main():
    ...
    output_rot90, middle_rot90 = net_rot(images_rot90)
    output = torch.nn.functional.softmax(output)
    output_rot90 = torch.nn.functional.softmax(output_rot90)
    images = torch.flatten(images, 1)
    images_rot90 = torch.flatten(images_rot90, 1)
    #Minimizing info nce maximizes MI
    # print(info_nce(middle, labels.reshape(-1, 1)))
    # print(info_nce(middle_rot90, labels.reshape(-1, 1)))
    print(info_nce(middle, middle))
    print(info_nce(middle, middle_rot90))
    # print(info_nce(images, images))
    # print(info_nce(images, images_rot90))
    # print(info_nce(output, output_rot90))
    # print(info_nce(images, labels.reshape(-1, 1)))
    # discretized = discretization(middle, 30)
    # discretized_rot90 = discretization(middle_rot90, 30)
    # print(mutual_info(discretized, discretized))
    # print(mutual_info(discretized_rot90, discretized))
    exit()
    p1 = np.array([mutual_info(discretized, images), mutual_info(discretized, labels.reshape(-1, 1))])
    p2 = np.array([mutual_info(discretized_rot90, images_rot90), mutual_info(discretized_rot90, labels.reshape(-1, 1))])
